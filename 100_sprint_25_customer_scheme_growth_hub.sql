-- Migration: 100_sprint_25_customer_scheme_growth_hub.sql
-- Description: Convert Dealer Growth Hub to automated Customer Scheme Growth Hub

-- 1. Add new configurations to dealer_schemes
ALTER TABLE public.dealer_schemes
ADD COLUMN IF NOT EXISTS near_slab_percentage DECIMAL(5,2) DEFAULT 10.0,
ADD COLUMN IF NOT EXISTS near_slab_bag_threshold NUMERIC(15,2) DEFAULT 20.0,
ADD COLUMN IF NOT EXISTS allow_stacking BOOLEAN DEFAULT true;

-- 2. Create the performance RPC
...
BEGIN
    ...
    purchasing_customers AS (
        ...
        JOIN public.sales_invoices si ON c.id = si.customer_id
        WHERE c.crm_status = 'Active' AND si.status = 'Completed'
    ),
    scheme_bags AS (
        SELECT 
        ...
        LEFT JOIN public.sales_invoices si ON pc.id = si.customer_id AND si.status = 'Completed' AND si.invoice_date >= s.start_date AND si.invoice_date <= s.end_date
    ...
    SELECT 
        ...
        CASE 
            ...
            THEN ROUND((se.current_net_bags / ROUND(se.historical_monthly_bags * (GREATEST(0, LEAST((CURRENT_DATE - s.start_date)::INT, (s.end_date - s.start_date)::INT)) / 30.0), 2)) * 100, 2)
            ELSE 0 
        ...
        CASE
            ...
            WHEN se.next_slab_id IS NOT NULL AND (
                ...
                ((se.next_slab_min_bags - se.current_net_bags) / se.next_slab_min_bags * 100) <= s.near_slab_percentage
            ) THEN 'near_next_slab'
