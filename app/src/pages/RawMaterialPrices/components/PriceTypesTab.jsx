...
export default function PriceTypesTab({ priceTypes, loading, onRefresh, showMessage }) {
  ...
  const handleOpenForm = (item = null, duplicate = false) => {
    if (item) {
      if (duplicate) {
        navigate('/raw-material-prices/configuration/price-types/new', { state: { duplicateFrom: item } });
      } else {
        navigate(`/raw-material-prices/configuration/price-types/${item.id}/edit`, { state: { priceType: item } });
      }
    } else {
      navigate('/raw-material-prices/configuration/price-types/new', { state: { nextOrder: (priceTypes?.length || 0) + 1 } });
    }
    setActiveMenuId(null);
  };
      ...
      <div className="flex-1 bg-white min-h-[400px]">
        {loading ? (
          <div className="p-6 space-y-4">
           ...
           <EmptyState icon={IndianRupee} title="No Price Types" description="Define types of prices you receive from brokers (e.g., Factory Delivery, Ex-Mill)." actionText="Add Price Type" onAction={() => handleOpenForm()} />
        ) : (
                          ...
                          {activeMenuId === pt.id && (
                            ...
                            >
                              <button 
                                ...
                                onClick={() => handleOpenForm(pt, true)}
                              >
