...
export default function UnitFormPage({ onRefresh, showMessage }) {
  ...
  useEffect(() => {
    if (isEdit) {
      if (location.state?.unit) {
        const item = location.state.unit;
        setFormData({
  ...
  const fetchUnit = async (unitId) => {
    try {
      ...
      setFormError('Failed to load unit details.');
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        const { error } = await supabase.from('rm_units').update(payload).eq('id', id);
        if (error) throw error;
      } else {
      ...
      showMessage('success', `Unit ${isEdit ? 'updated' : 'added'} successfully.`);
      if (onRefresh) onRefresh();
      navigate('/raw-material-prices/configuration/units');
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center text-xs font-medium text-secondary mb-4 uppercase tracking-wider">
        ...
        <ChevronRight size={14} className="mx-1 opacity-50" />
        <Link to="/raw-material-prices/configuration/units" className="hover:text-primary transition-colors">Units</Link>
        <ChevronRight size={14} className="mx-1 opacity-50" />
        <span className="text-primary">{isEdit ? 'Edit Unit' : 'Add Unit'}</span>
      </div>

      <div className="page-header">
        <div style={{display: 'flex', alignItems: 'center', gap: '1rem'}}>
          <Link to="/raw-material-prices/configuration/units" className="btn-icon" title="Back to Units">
            <ArrowLeft size={24} />
          </Link>
          <div>
            <h1>{isEdit ? 'Edit Unit' : 'Add Unit'}</h1>
            <p className="text-secondary" style={{marginTop: '0.25rem'}}>
              Configure unit settings for raw material measurements.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel" style={{maxWidth: '800px', padding: '2rem'}}>
        ...
        <form id="unit-form" onSubmit={handleSave}>
          ...
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: '1.5rem', marginTop: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <label style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', padding: '1rem', background: 'var(--bg-surface-hover)', borderRadius: 'var(--radius-md)'}}>
              <div>
                ...
                <span style={{fontSize: '0.875rem', color: 'var(--text-secondary)', display: 'block'}}>Make this unit available for selection in prices.</span>
              </div>
          ...
          <div style={{display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '2rem'}}>
            <Link to="/raw-material-prices/configuration/units" className="btn btn-secondary">
              Cancel
            </Link>
            <button 
              type="submit" 
