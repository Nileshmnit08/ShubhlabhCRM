...
export default function ActiveSchemesList({ schemes, onBack }) {
  return (
    <div className="animate-fade-in" style={{ padding: '1.5rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem', borderBottom: '1px solid var(--border)', paddingBottom: '1rem' }}>
        ...
        <h2 style={{ margin: 0 }}>Active Schemes ({schemes.length})</h2>
      </div>

      {schemes.length === 0 ? (
        <div style={{ padding: '3rem', textAlign: 'center' }} className="text-muted">No active schemes found.</div>
      ) : (
                ...
                return (
                  <tr key={s.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '1rem' }}>
                      <span className={`badge ${remaining <= 15 ? 'badge-danger' : 'badge-primary'}`}>
                        {remaining} Days
                      </span>
                    </td>
                    <td style={{ padding: '1rem', textAlign: 'center' }}>
                      <Link to="/settings?tab=dealer_schemes" className="btn btn-secondary btn-sm" title="Edit Scheme">
                        <Edit2 size={16} /> Edit
                      </Link>
                    </td>
                  </tr>
