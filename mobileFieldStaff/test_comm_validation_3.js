...
async function validate() {
  ...
  let { data: allCalls } = await supabase.from('v_crm_call_events_enriched').select('direction, call_type');
  
  let inc = allCalls.filter(c => c.direction === 'INCOMING').length;
  let out = allCalls.filter(c => c.direction === 'OUTGOING').length;
  let unk = allCalls.filter(c => c.direction === 'UNKNOWN').length;

  console.log('Incoming:', inc);
  console.log('Outgoing:', out);
  console.log('Unknown Direction:', unk);
}
validate();
