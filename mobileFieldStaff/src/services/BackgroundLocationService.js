...
export const stopBackgroundLocationTracking = async () => {
  ...
  if (hasStarted) {
    ...
    // Clear geofence state for the current session to prevent stale evaluations next time
    const { data: { session } } = await supabase.auth.getSession();
    if (session) {
      await AsyncStorage.removeItem(getGeofenceStorageKey(session.user.id));
    }
  }
};
